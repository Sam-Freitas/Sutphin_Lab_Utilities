...
response = run_command('cd ' + ssh_details['cd_path_for_Data'])
response = run_command('pwd')

# response = run_command('rsync -avzh --update --info=progress2 '+ ssh_details['rsync_get_path'] + ' .')
# response = run_command('rsync -avzh --update --exclude="*processed_img_data*" --info=progress2 samfreitas@filexfer.hpc.arizona.edu:/groups/sutphin/_Data .')
response = run_command("rsync -rlvzh --update --exclude='*processed_img_data*' --info=progress2 samfreitas@filexfer.hpc.arizona.edu:/groups/sutphin/_Data .")
time.sleep(1)
